...
def test_build_simple_road(setup):
    ...
    assert tile1.has_building(BuildingType.ROAD) is True , "Road should be built on visible and self claimed tile"
    assert tile1.get_owner() != OWNER_DEFAULT_TILE, "Bulding a road should not claim the tile"
...
def test_build_simple_bridge(setup):
    ...
    assert tile1.has_any_building() is True
    assert tile1.has_building(BuildingType.BRIDGE) is True , "Bridge should be built on visible and self claimed tile"
    assert tile1.get_owner() != OWNER_DEFAULT_TILE, "Building a Bridge should not claim the tile"
