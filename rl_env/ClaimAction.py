...
class ClaimAction(Action):
    ...
    def validate(self, env) -> bool:
        ...
        if not env.action_manager.check_position_on_map(self.claim_position):
            print(f"Agent {self.agent.id}: Claim position {self.claim_position} is out of bounds.")
            return False
    ...
    def execute(self, env) -> int:
        ...
        print(f"Agent {self.agent.id}: Claimed tile at {self.claim_position}. Reward: {reward}")
        return reward
